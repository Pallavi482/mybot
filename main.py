...
def handle_photo(message):
    ...
    for admin_id in ADMIN_LIST:
        try:
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("✅ Grant VIP Access", callback_data=f"approve_{message.from_user.id}"))
            bot.send_photo(
                admin_id, 
...
def approve_user(call):
    ...
    try:
        ...
        bot.answer_callback_query(call.id, "✅ User ko access bhej diya gaya hai!")
    except Exception as e:
